...
RSpec.describe Elements::Reinforcement::StraightLongitudinal do
  ...
  describe '#add_straight_longitudinal_layer' do
    before do
      reinforcement.add_straight_longitudinal_layer(start_location:, end_location:)
    end

    it 'adds straight_longitudinal_layer' do
      expect(reinforcement.instance_variable_get(:@straight_longitudinal_layers).size).to eq(1)
    end

    it 'sets the start_location of the straight_longitudinal_layer' do
      expect(reinforcement.instance_variable_get(:@straight_longitudinal_layers).first.start_location).to eq(start_location)
    end

    it 'sets the end_location of the straight_longitudinal_layer' do
      expect(reinforcement.instance_variable_get(:@straight_longitudinal_layers).first.end_location).to eq(end_location)
    end
  end
  ...
  describe '#modify_z_base' do
    describe 'when there is no straight_longitudinal_layers' do
      ...
      it 'keeps straight_longitudinal_layers empty' do
        expect(reinforcement.instance_variable_get(:@straight_longitudinal_layers)).to be_empty
      end
    end
    ...
    describe 'when there are straight_longitudinal_layers' do
      ...
      it 'updates value_z at start_location of straight_longitudinal_layers' do
        expect(reinforcement.instance_variable_get(:@straight_longitudinal_layers).first.start_location.value_z).to eq(500)
      end

      it 'updates value_z at end_location of straight_longitudinal_layers' do
        expect(reinforcement.instance_variable_get(:@straight_longitudinal_layers).first.end_location.value_z).to eq(500)
      end
    end
  end
  ...
  describe '#add' do
    ...
    it 'adds rebar' do
      expect(reinforcement.instance_variable_get(:@straight_longitudinal_layers).first.rebar).to eq(rebar)
    end
  end
  ...
  describe '#change' do
    ...
    it 'changes rebar' do
      id_of_straight_longitudinal_layer_to_change = reinforcement.instance_variable_get(:@straight_longitudinal_layers).first.id
      reinforcement.change(id_of_straight_longitudinal_layer_to_change:, number_of_rebars: 15, rebar: rebar_changed, length: 900)
      expect(reinforcement.instance_variable_get(:@straight_longitudinal_layers).first.rebar).to eq(rebar_changed)
    end
  end
end
