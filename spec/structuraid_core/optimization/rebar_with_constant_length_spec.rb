...
RSpec.describe StructuraidCore::Optimization::RebarWithConstantLength do
  ...
  let(:maximum_rebar_spacing) { 200 }
  let(:minimum_spacing) { 100 }
  ...
  describe 'with big required_reinforcement_area' do
    let(:required_reinforcement_area) { 100_000 }
    ...
    describe '#run' do
      ...
      it 'returns a value at :rebar key' do
        expect(optimizator.run.rebar).to be(nil)
      end

      it 'returns a value at :amount_of_rebars key' do
        expect(optimizator.run.amount_of_rebars).to be(nil)
      end
    end
